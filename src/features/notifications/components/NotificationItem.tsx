...
const iconConfig: Record<NotificationType, { icon: React.ReactNode; bg: string }> = {
  ...
  territory_claimed: { icon: <MapPin size={15} strokeWidth={2} className="text-[#E8435A]" />,    bg: 'bg-[#F9E4E7]' },
  ...
  follow:            { icon: <UserPlus size={15} strokeWidth={2} className="text-[#E8435A]" />,  bg: 'bg-[#F9E4E7]' },
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !notification.read ? 'bg-[#F9E4E7]/40' : 'bg-white'
      )}
    >
      ...
      {!notification.read && (
        <div className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-[#E8435A] mt-2" />
      )}
    </button>
  )
}
