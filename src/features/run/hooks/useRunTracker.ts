...
import type { Location, LiveRunData, Territory } from '@shared/types/index';
...
export const useRunTracker = () => {
  ...
  const checkTerritoryIntersection = useCallback((currentLocation: Location, territories: Territory[]): string[] => {
    ...
    territories.forEach(territory => {
      ...
      const { polygon } = territory as any;
      if (polygon.length >= 4) {
        const minLat = Math.min(...polygon.map((p: {lat: number}) => p.lat));
        const maxLat = Math.max(...polygon.map((p: {lat: number}) => p.lat));
        const minLng = Math.min(...polygon.map((p: {lng: number}) => p.lng));
        const maxLng = Math.max(...polygon.map((p: {lng: number}) => p.lng));
