...
import { fetchActiveStoriesForUsers, type Story } from '@shared/services/storiesService';
import {
  Bell,
  MessageSquare,
  MapPin,
...
interface GetFeedRow {
  id: string; user_id: string; username: string | null; content: string | null;
  distance_km: number | null; territories_claimed: number | null;
  likes: number; comment_count: number | null; created_at: string;
}

interface ProfileRow {
  id: string; username: string | null; level: number | null;
  total_distance_km: number | null; total_territories_claimed: number | null;
  streak_days: number | null; created_at: string;
}

interface ContactsManager {
  select(properties: string[], options?: { multiple?: boolean }): Promise<{ name: string[] }[]>;
}
...
// Normalize real GPS {lat,lng}[] from DB into [x,y][] in 0-100 space for RouteMapHero
function normalizeRoute(raw: { lat: number; lng: number }[] | null): [number, number][] | null {
  ...
  return raw.map(p => [
    ((p.lng - minLng) / lngRange) * 96 + 2,       // x: 2–98
    (1 - (p.lat - minLat) / latRange) * 96 + 2,   // y: 2–98 (flipped for SVG)
  ]);
}
...
function buildPost(row: {
  ...
  runs: { gps_points: { lat: number; lng: number }[] | null; duration_sec: number | null; avg_pace: string | null } | null;
}): Post {
  ...
  return {
    ...
    activity: {
      ...
      route: normalizeRoute(run?.gps_points ?? null),
    },
    kudos: row.likes,
    kudosUsers: [],
    comments: 0,
...
export default function Feed() {
  ...
  const [crewStories, setCrewStories] = useState<Map<string, Story[]>>(new Map());
  ...
  const loadFeed = async (tab: FeedTab) => {
    ...
    if (tab === 'following') {
      ...
      if (!error && data) {
        setPosts((data as GetFeedRow[]).map(row => ({
          id: row.id,
          userId: row.user_id,
          user: {
      ...
      const { data } = await supabase
        .from('feed_posts')
        .select('id, user_id, content, distance_km, territories_claimed, likes, created_at, profiles(username, level), runs(gps_points, duration_sec, avg_pace)')
        .order('created_at', { ascending: false })
        .limit(30);
      if (data) setPosts(data.map(row => buildPost(row as unknown as Parameters<typeof buildPost>[0])));
    }
  ...
  const loadRunners = async () => {
    ...
    setRunners((data ?? [] as ProfileRow[]).map((row: ProfileRow) => {
      const name = row.username ?? 'Runner';
      const badge: SuggestedRunner['badge'] =
        (row.total_territories_claimed ?? 0) >= 50 ? 'conqueror'
        : (row.streak_days ?? 0) >= 7 ? 'streak'
        : (row.level ?? 0) >= 30 ? 'top10'
        : undefined;
      return {
        id: row.id, name,
  ...
  useEffect(() => {
    const ids = [...following];
    if (ids.length === 0) { setCrewStories(new Map()); return; }
    fetchActiveStoriesForUsers(ids).then(setCrewStories).catch(() => {});
  }, [following]);
        ...
        <div className="px-5 pt-6 pb-5">
          <div className="flex items-start gap-4">
            <div className="relative">
              <div
                style={{ background: avatarColor(profileRunner.name) }}
            ...
            <div style={{
              ...
              >
                {undismissed.slice(0, 8).map(runner => {
                  ...
                  const sc = avatarColor(runner.name);
                  return (
                    <div
                      key={runner.id}
            ...
            <>
              ...
              >
                {posts.map(post => {
                  ...
                  const sc = avatarColor(post.user.name);
                ...
                {followedRunners.length > 0 ? (
                  <div className="flex gap-4 px-4 overflow-x-auto scrollbar-none pt-3 pb-2">
                    {followedRunners.map(r => {
                      ...
                      const userStories = crewStories.get(r.id) ?? [];
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            haptic('light');
                            if (hasStory) {
                              // Build groups from all followed runners who have stories
                              const groups = followedRunners
                                .filter(fr => (crewStories.get(fr.id) ?? []).length > 0)
                                .map(fr => ({ userId: fr.id, userName: fr.name, stories: crewStories.get(fr.id)! }));
                              const initialGroupIndex = Math.max(0, groups.findIndex(g => g.userId === r.id));
                              navigate('/story-viewer', { state: { groups, initialGroupIndex } });
                            } else {
                              setProfileRunner(r);
                            }
                          }}
                          ...
                          <div className="relative">
                            ...
                            <div
                              style={{
                                ...
                                boxShadow: !hasStory && hasNew ? `0 0 0 2px ${T.red}` : !hasStory ? `0 0 0 1.5px ${T.border}` : undefined,
                                position: 'relative', zIndex: 1,
                                outline: hasStory ? '2.5px solid white' : undefined,
                                outlineOffset: hasStory ? '0px' : undefined,
                              }}
                    ...
                    <div key={post.id} style={{ background: T.cardBg, borderBottom: `0.5px solid ${T.border}` }} className="active:bg-[#FAFAF8] transition-colors duration-100">
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 20px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            {post.kudosUsers.slice(0, 3).map((u, i) => (
                              <div key={i} style={{ width: 18, height: 18, borderRadius: '50%', background: avatarColor(u.name), border: '1.5px solid white', marginLeft: i === 0 ? 0 : -5, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7, color: 'white', fontWeight: 500 }}>
                                {u.initial}
                              </div>
                            ))}
                            {(post.kudos > 0 || isKudos) && (
                    ...
                    <div className="divide-y divide-gray-50">
                      {filteredSearchResults.map(runner => (
                        <button key={runner.id} className="w-full flex items-center gap-3 px-4 py-3 active:bg-gray-50 transition text-left"
                          ...
                          <div style={{ background: avatarColor(runner.name) }}
                      ...
                      {visiblePopular.map(runner => {
                        ...
                        return (
                          ...
                          >
                            <div className="relative">
                              <div
                                style={{ background: avatarColor(runner.name), boxShadow: `0 0 0 1.5px ${T.border}` }}
                                className="w-[54px] h-[54px] rounded-full flex items-center justify-center text-[17px] font-bold text-white"
                              >
                                {runner.initial}
                              </div>
                    ...
                    <div style={{ margin: '0 20px', background: T.cardBg, border: `0.5px solid ${T.border}`, borderRadius: 4, overflow: 'hidden' }}>
                      {visiblePopular.map((runner, idx) => (
                        <button key={runner.id} onClick={() => { setProfileRunner(runner); haptic('light'); }}
                          ...
                          className="active:bg-[#FAFAF8] transition-colors">
                          <div style={{ width: 40, height: 40, borderRadius: '50%', background: avatarColor(runner.name), flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 400, color: 'white' }}>
                            {runner.initial}
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {(showAllNearby ? visibleNearby : visibleNearby.slice(0, 3)).map((runner, idx) => (
                      <button key={runner.id} onClick={() => { setProfileRunner(runner); haptic('light'); }}
                        ...
                        <div style={{ position: 'relative', flexShrink: 0 }}>
                          <div style={{ width: 40, height: 40, borderRadius: '50%', background: avatarColor(runner.name), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 400, color: 'white' }}>
                            {runner.initial}
                          </div>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div>
                        {contactsOnRunivo.map((contact, idx) => (
                          <div key={contact.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderTop: idx > 0 ? `0.5px solid ${T.mid}` : 'none' }}>
                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: avatarColor(contact.name), flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 400, color: 'white' }}>
                              {contact.initial}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div>
                      ...
                      <div>
                        {contactsNotOnRunivo.map((contact, idx) => (
                          <div key={contact.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderTop: idx > 0 ? `0.5px solid ${T.mid}` : 'none' }}>
                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: avatarColor(contact.name), flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 400, color: 'white', opacity: 0.5 }}>
                              {contact.initial}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
