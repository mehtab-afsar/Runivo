...
const STEPS: Step[] = [
  {
    icon: <MapPin className="w-5 h-5 text-[#D93518]" strokeWidth={2} />,
  ...
  {
    icon: <Flag className="w-5 h-5 text-[#D93518]" strokeWidth={2} />,
        ...
        >
          <div className="bg-white rounded-2xl p-4 shadow-xl border border-gray-100">
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-xl bg-[#FEF0EE] flex items-center justify-center shrink-0">
                {step.icon}
              </div>
              ...
              {STEPS.map((_, i) => (
                <div
                  ...
                  className={`h-1 rounded-full transition-all ${
                    i === stepIndex ? 'w-4 bg-[#D93518]' : 'w-1 bg-gray-200'
                  }`}
                />
              ))}
            </div>
