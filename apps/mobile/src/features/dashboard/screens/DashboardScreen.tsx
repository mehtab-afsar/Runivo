...
import { Bell, Map, Zap, Activity, Flame, Target } from 'lucide-react-native';
...
function HeroCarousel({
  ...
  const containerW = SCREEN_W - 44;
  const cardW      = containerW - 16;
  ...
  const noNutrition = caloriesConsumed === 0 && calorieGoal === 2000;
  ...
  return (
    <View style={{ marginHorizontal: 22, marginBottom: 18 }}>
      <ScrollView
        ...
        onScroll={e => setPage(Math.round(e.nativeEvent.contentOffset.x / (cardW + 8)))}
        scrollEventThrottle={16}
      >
        {/* Card 1: Weekly Goal */}
        <View style={[hc.card, { width: cardW }]}>
          <View style={hc.cardLabelRow}>
            <Target size={10} color={C.t3} strokeWidth={1.5} />
        ...
        <Pressable style={[hc.card, { width: cardW }]} onPress={onGoCalories}>
          <View style={hc.cardLabelRow}>
            <Flame size={10} color={C.t3} strokeWidth={1.5} />
            <Text style={hc.cardLabel}>  TODAY'S CALORIES</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            <View style={hc.flameCircle}>
              <Flame size={24} color="rgba(249,115,22,0.8)" strokeWidth={1.5} />
            </View>
            <View style={{ flex: 1 }}>
              <View style={hc.row}>
          ...
          <Text style={hc.sub}>{noNutrition ? 'Set up nutrition goal →' : 'Tap to log food →'}</Text>
        </Pressable>
      </ScrollView>
...
const hc = StyleSheet.create({
  card:         { backgroundColor: C.white, borderRadius: 16, borderWidth: 0.5, borderColor: C.border, padding: 16, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 8, shadowOffset: { width: 0, height: 2 } },
  cardLabelRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  cardLabel:    { fontFamily: 'Barlow_500Medium', fontSize: 10, letterSpacing: 1, color: C.t3 },
  ...
  dotActive:    { backgroundColor: C.black, width: 16 },
});
      ...
      <ScrollView style={ss.fill} contentContainerStyle={{ paddingBottom: 100, paddingTop: insets.top }} showsVerticalScrollIndicator={false}>
        ...
        <View style={ss.section}>
          <View style={ss.sectionHead}>
            <View style={ss.sectionTitleRow}>
              ...
              <Text style={ss.sectionTitle}>  EMPIRE</Text>
            </View>
            <Pressable onPress={() => go('TerritoryMap')}><Text style={ss.sectionAction}>View map →</Text></Pressable>
          </View>
        ...
        <View style={ss.section}>
          <View style={ss.sectionHead}>
            <View style={ss.sectionTitleRow}>
              ...
              <Text style={ss.sectionTitle}>  MISSIONS</Text>
            </View>
            <Pressable onPress={() => go('Missions')}><Text style={ss.sectionAction}>Change →</Text></Pressable>
          </View>
        ...
        <View style={ss.section}>
          <View style={ss.sectionHead}>
            ...
            <Pressable onPress={() => go('History')}><Text style={ss.sectionAction}>See all →</Text></Pressable>
          </View>
