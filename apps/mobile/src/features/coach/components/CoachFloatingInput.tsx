    ...
    <Animated.View style={[ss.outer, { paddingBottom: Math.max(insets.bottom, 8) + 4, transform: [{ translateY: slideY }], opacity }]}>
      <View style={ss.pill}>
        ...
        <Pressable
          ...
          hitSlop={8}
        >
          <ArrowUp size={16} color={canSend ? '#fff' : 'rgba(10,10,10,0.22)'} strokeWidth={2.5} />
        </Pressable>
      </View>
    </Animated.View>
  );
...
const ss = StyleSheet.create({
  ...
  input:        { flex: 1, fontFamily: 'DMSans_400Regular', fontSize: 14, color: '#0A0A0A', maxHeight: 120, paddingVertical: 8, paddingHorizontal: 4 },
